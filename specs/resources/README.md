...
The desktop resolver produces text and structured-data handles. It accepts inline PNG, JPEG, GIF, WebP, MP3, Ogg, WAV, MP4 and WebM data URIs for media. Other media values become `unavailable` handles. Network resource resolution remains disabled until a host resource-fetch adapter routes it through the bounded HTTP transport and `network.request` authorization.
